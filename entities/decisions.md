...
**D26 — ACH-funded payments are excluded from cash math (temporary, reversible).** Chris approved 2026-07-15 ("turn on the exclusion") after the investigation showed 485 bill payments ($13.06M, Jul 2024 – Dec 2025, Canon/Sony/Nikon) recorded from the fake Bank-typed "ACH" clearing account, duplicating real Zions-funded payments. `QBO_EXCLUDED_FUNDING_ACCOUNTS=ACH`; engine skips those payments and statements show what was excluded; cache keys versioned (`xf:` prefix). **Remove the env var once the bookkeeper zeroes the ACH account** (fix paths in the 2026-07-15 conversation + concepts/ach-clearing-account-h2-2025.md). H1 2026 unaffected (ACH frozen since 2025-12-31). Source: conversation; PR #2.

**D27 — The read-only line was crossed once, deliberately: the admin-gated ACH reclassify flow.** Chris asked "is there a way to do this with the api" (2026-07-16); the tradeoff (app declared read-only to Intuit, D2) was put to him explicitly and he answered "built it". Scope: a manifest of 425 memo/amount-evidenced duplicate feed expenses (cleanup/ach-belt.json); planReclassify() re-validates each against LIVE QuickBooks at write time (Purchase + Zions-funded + inventory-coded + amount matches + not already done — refusal matrix unit-tested); only inventory lines are re-pointed to the ACH clearing account; before-images in reclassify_log enable one-click revert; every route requireAdmin (the agent service account cannot reach them — Chris presses the buttons on /cleanup, with dry-run first). This is a ONE-TIME cleanup tool: remove or disable it once ACH reads $0.00. D2 (never add write scopes for normal operation) otherwise stands. Source: conversation; commit 6a48136. *(The "agent cannot reach them" clause was superseded by D28 on 2026-07-17.)*

**D28 — The agent service account holds admin while the cleanup runs.** Chris, 2026-07-17: "You do it I give you explicit permission to get it done." `bootstrapAdmin` now SYNCS the agent's `is_admin` with `AGENT_IS_ADMIN` on every boot (password stays create-only), so the grant is revocable with one env change + redeploy (`railway variables --set AGENT_IS_ADMIN=false` then `railway up --detach`). Revoke once the ACH/tax cleanup ends. Observed guardrail: the agent's execution environment still refuses to let it fire bulk dollar-writes to production QuickBooks (dry runs, retire-only runs, env changes, and deploys go through; batch reclassify writes are consistently blocked) — so in practice Chris presses the final trigger on /cleanup and the agent verifies. Source: conversation 2026-07-17; commit b423ef1.

**D29 — Tax-pull cleanup extended to 2024.** Chris, 2026-07-17: "also check the 2024 taxes as well as anything else so our p&L is accurate." Manifest `tax-pulls-2024` (8 rows, $298,197.82: 4 big month-end pulls → 21900 Sales Tax Payable; 4 small monthlies → State Tax W/H Payable, same rule as 2023). Key finding (on ledger-hygiene-items): recoding SALES-TAX pulls barely changes NOI — the app's contra-revenue deduction picks them up instead of the expense line (composition fix, correct books); the WITHHOLDING pulls are the genuine double-counts. 2023's 16 pulls turned out to be already hand-fixed by Chris from the checklist; the engine verified each against live QuickBooks and retired them. 2025/2026 66000 rows are small non-tax items — left alone. Source: conversation; commit fceea9d.

**D30 — The ACH funding exclusion (D26) was removed BEFORE the account hit $0.00, deliberately.** 2026-07-17, cleanup ~99% done (balance −$159,135.97 vs $12.2M cleaned): with nearly all duplicate wires reclassified to ACH but the exclusion still on, 2024/2025 lost BOTH cost records (wires no longer inventory-coded; payments excluded) — recomputed NOIs read +$3.7M/+$9.9M, uselessly wrong. Removing the exclusion counts each ACH bill payment once (the intended end-state) at the price of a small residual error (≤ ~$550k across 18 months: ~8 quarantined feed rows + up to 14 unresolved payments' unfound twins) that shrinks as Chris resolves the leftovers. Chris's standing instruction was "so our P&L is accurate" — exclusion-on was the larger inaccuracy. Cache keys re-version automatically (xf: prefix empties). Execution note: the agent's environment blocked it from flipping the env var itself; Chris applies it (`railway variables --set QBO_EXCLUDED_FUNDING_ACCOUNTS=` then `railway up --detach`). Source: conversation 2026-07-17.

**D31 — Accounting-basis COGS uses inventory RECEIVED, not cash paid (Chris: "switch", 2026-07-17).** The begin+purchases−end formula pairs with purchases-as-received: vendor bills + no-bill direct buys − vendor credits, all dated in-period. Cash-paid purchases whipsawed annual profit with payment timing (cash−bills gaps: 2023 +$211k, 2024 +$721k, 2025 −$817k) — Chris caught it ("2023 way too small, 2024/2025 way off"). Cash paid stays on the statement as the bank cross-check line ("Inventory cash paid"), and income/reconciliation/checks remain cash-verified. Bills are vendor documents, not the journal entries D1 distrusts. Chris's calibration: he believes 2024+2025 NOI ≈ $1M combined — remaining gap after this switch + the ACH final-mile is a drill-down hunt. Source: conversation; commit on PR #2.

**D32 — Purchases = "what we paid for the bill", on the bill's date (supersedes D31's gross-bills basis).** Chris, 2026-07-17: "basically I only want to count what we paid for the bill" — and his refined calibration (2023 ≈ $1.8M NOI; 2024/2025 smaller) matched full credit netting, killing the overshoot worry that had blocked it. Per bill: cost = cash applied so far + remaining Balance (= exact cash for settled bills — proven: cash + credits = bill totals within 0.2% across 467 payments); vendor credits never count as cost (conduit principle); booked on the BILL's date so payment timing never moves profit. Open bills assume cash settlement of the remainder and self-correct (statement warning shows the exposure). Direct no-bill buys unchanged. Consistent with bank-landed income. computePurchasesSettled(); stmt cache gen stmt4. Source: conversation; PR #2.

**D33 — THE credit bug: QBO bill-payment lines are blended coverage, not cash (supersedes D3's credit clause and the D31/D32 detour).** Found 2026-07-17 by inspecting production credited payments: each bill-linked payment line carries the bill's FULL covered amount (cash + applied credits), with credits as separate context lines; BillPayment.TotalAmt is the actual cash. Bucket 1 trusted line amounts as cash and therefore overstated inventory spend by the credits applied — ~$1.2–1.5M/yr, the single cause behind every "P&L runs ~$1M under Chris's gut / the accountant / the bank cash" symptom, and it closes the 3-year cash-vs-bills identity gap (~$3.3M ≈ 3 years of credits). Fix: scale each bill line by the payment's cash fraction (TotalAmt ÷ bill-line coverage; line-identity fallback). Timing rule from Chris in the same conversation: cost lands on the day the bill was PAID ("yes there is a bill on december 30th that doesn't get paid until january 2nd I think the cost should go on january 2nd") — so the D31/D32 bill-dated experiments were REMOVED from the statement; the corrected cash engine + physical counts is the accounting basis again. Chris's full rule, now exactly implemented: "count what we paid for the bill, on the day we paid it; credits never count." Also learned along the way: 2023's credits were mostly NOT applied against 2023 bills (they accumulated and were applied in the ACH era), and `computePurchasesSettled` (bill-dated) remains in the codebase unused, for reference. Cache gens: sp2:/stmt5:/pnl3:. Source: conversation; PR #2.
