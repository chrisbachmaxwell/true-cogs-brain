...
## 1. Sign-in gate live — CURRENT GOAL → [../goals/01-auth-gate.md](../goals/01-auth-gate.md)
The dashboard shows Pictureline's full financials and is open to anyone with the URL. All code exists; it activates the moment `RESEND_API_KEY` is set in Railway. **(Chris: create Resend account + key.)** Then verify end-to-end: unauthenticated requests blocked, magic link arrives, sign-in works, QBO `/connect`//`/callback` still open.

## 2. Rotate the Railway account token **(Chris)**
...
## 3. Monthly close routine (recurring, small)
...
## 4. Bank-ledger CSV reconciliation (parked by Chris: "I'll ask that later")
...
## 5. Boise credit-memo audit (parked by Chris: "we can look into that later")
...
## 6. Bookkeeper hand-off of ledger-hygiene items
